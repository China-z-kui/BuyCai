...
//删除接口
 router.delete('/s/:uid',(req,res,next)=>{
...
//登录接口
 router.post('/login',(req,res,next)=>{
    ...
    pool.query(`select * from  user where uname= ? and upwd=?`,[req.body.uname,req.body.upwd],(err,r)=>{
        if (err) {
...
router.put('/change',(req,res,next)=>{
    console.log(req.body)
    pool.query('update  user set?  where uid=?',[req.body,req.body.uid],(err,r)=>{
		if(err){
			return next(err)
		}
		console.log(r)
